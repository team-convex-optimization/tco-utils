...
static struct reg_value ov5645_setting_60fps_VGA_640_480[] = {
	{0x3103, 0x11, 0, 0},
	//	Comment this out in order for AF to work
	//	{0x3008, 0x82, 0, 0},
	{0x3008, 0x42, 0, 0},
...
static struct ov5645_mode_info ov5645_mode_info_data[3][ov5645_mode_MAX + 1] = {
	...
	{
		{ov5645_mode_VGA_640_480, SUBSAMPLING, 640, 480,
		ov5645_setting_60fps_VGA_640_480,
		ARRAY_SIZE(ov5645_setting_60fps_VGA_640_480)},
		{ov5645_mode_NTSC_720_480, -1, 0, 0, NULL, 0},
		{ov5645_mode_720P_1280_720, SUBSAMPLING, 1280, 720,
...
static int OV5645_set_shutter(int shutter)
{
	...
	pr_crit("Setting line period to %d \n", shutter);
											...
											enum ov5645_mode mode)
{
	...
	if ((binning_on()) && (mode != ov5645_mode_720P_1280_720) && (mode != ov5645_mode_1080P_1920_1080))
		prev_shutter *= 2;
	...
	if (average > AE_low && average < AE_high)
	{
		/* in stable range */
		cap_gain16_shutter =
			prev_gain16 * prev_shutter * cap_sysclk / prev_sysclk * prev_HTS / cap_HTS * AE_Target / average;
	}
	else
	{
		cap_gain16_shutter =
			prev_gain16 * prev_shutter * cap_sysclk / prev_sysclk * prev_HTS / cap_HTS;
	}
							...
							enum ov5645_mode mode, enum ov5645_mode orig_mode)
{
	...
	if ((mode > ov5645_mode_MAX || mode < ov5645_mode_MIN) && (mode != ov5645_mode_INIT))
	{
	...
	else /*TODO: If smth wrong, chng this plz */
	{
									  ...
									  struct v4l2_subdev_frame_interval_enum *fie)
{
	...
	for (i = 0; i < ARRAY_SIZE(ov5645_mode_info_data); i++)
	{
		...
		{
			if (fie->width == ov5645_mode_info_data[i][j].width && fie->height == ov5645_mode_info_data[i][j].height && ov5645_mode_info_data[i][j].init_data_ptr != NULL)
			{
				count++;
			}
...
static int init_device(void)
{
	...
	pr_emerg("Setting mclk to %d MHz\n", tgt_xclk / 1000000);
					...
					struct v4l2_dbg_register *reg)
{
	...
	ret = ov5645_read_reg(reg->reg, &val);
	if (!ret)
		reg->val = (__u64)val;
					...
					const struct v4l2_dbg_register *reg)
{
	...
	pr_crit("Writting register %d val %d\n", reg->reg, reg->val);

	return ov5645_write_reg(reg->reg, reg->val);
}
#endif
